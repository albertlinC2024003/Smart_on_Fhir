...
import {UrlPath} from "../utils/module/PathListener.tsx"; // 路徑依你的專案調整
    ...
    return (
        <Box className="flex flex-col items-center justify-center h-screen w-full text-2xl">
            ...
            <Box className={"flex mt-4 gap-2"}>
                ...
                {/*}} />*/}
                <CustomButton text={"Fhir資料"} onClick={() => handleNavigate(UrlPath.FHIR_GETTER)} />
                <CustomButton text={"測試CQL"} onClick={() => handleNavigate(UrlPath.CQL_GETTER)} />
                <CustomButton text={"Fhir UI"} onClick={() => handleNavigate(UrlPath.FHIR_REACT)} />
            </Box>
        </Box>
    )
}
